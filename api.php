...
//    echo "<a href=\"https://lbmgaming.eu.auth0.com/authorize?response_type=token&client_id=Xt4G1EO4SgEj3aDw7e2VZXVlFjxvcnfx&connection=CONNECTION&redirect_uri=\">Login</a>";
    ...
    header('Location: ' . $root);
    die();
} elseif($method == 'POST' && $action == 'addmenut') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
    ...
    die();
} elseif($method == 'POST' && $action == 'removemenut') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
            $menu = json_decode($dbUser->menut);
    ...
    die();
} elseif($method == 'POST' && $action == 'addbtnt') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
    ...
    die();
} elseif($method == 'POST' && $action == 'removebtnt') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
            $menu = json_decode($dbUser->btnt);
    ...
    die();
} elseif($method == 'POST' && $action == 'addmenue') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
    ...
    die();
} elseif($method == 'POST' && $action == 'removemenue') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
            $menu = json_decode($dbUser->menue);
    ...
    die();
} elseif($method == 'POST' && $action == 'addbtne') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
    ...
    die();
} elseif($method == 'POST' && $action == 'removebtne') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
            $menu = json_decode($dbUser->btne);
    ...
    die();
} elseif($method == 'POST' && $action == 'addmenude') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
            $menu = json_decode($dbUser->menude);
            if(!in_array($id, $menu)) {
                $menu[] = $id;
                $db->update('users', ['menude' => json_encode($menu)], ['uid' => $user['uid']]);
            }
        }
    }

    die();
} elseif($method == 'POST' && $action == 'removemenude') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
            $menu = json_decode($dbUser->menude);
    ...
    die();
} elseif($method == 'POST' && $action == 'addbtnde') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
            $menu = json_decode($dbUser->btnde);
            if(!in_array($id, $menu)) {
                $menu[] = $id;
                $db->update('users', ['btnde' => json_encode($menu)], ['uid' => $user['uid']]);
            }
        }
    }

    die();
} elseif($method == 'POST' && $action == 'removebtnde') {
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        if($dbUser) {
            $menu = json_decode($dbUser->btnde);
    ...
    die();
} elseif($method == 'GET' && $action == 'menut') {
    header('Content-Type: application/json');
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
    ...
    echo json_encode($user);
    die();
} elseif($method == 'GET' && $action == 'menue') {
    header('Content-Type: application/json');
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
    ...
    echo json_encode($user);
    die();
} elseif($method == 'GET' && $action == 'menude') {
    header('Content-Type: application/json');
    if($user = session('user')) {
        $dbUser = $db->get_row("SELECT * FROM users WHERE uid = {$user['uid']}");
        $user['menu'] = ($dbUser) ? json_decode($dbUser->menude) : [];
        $user['btn'] = ($dbUser) ? json_decode($dbUser->btnde) : [];
    }
    echo json_encode($user);
    die();
} else {
